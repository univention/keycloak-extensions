...
class ActionDelegate:
    ...
    def __init__(self, state, fails, condition):
        ...
        self.state = state
        self.proxy = f"http://{self.kc_proxy}/proxy-api"
        ...
        if (self.condition == "ipAddress"):
            ...
            self.proxy_condition = state.get("ipAddress")
    ...
    def trigger(self):
        ...
        if (self.fails > 5 and self.condition == "code_id"):
            payload = {"action": "block", "condition": self.condition_value,
                       "value": self.proxy_condition, "valid_until": datetime.timestamp(datetime.now() + timedelta(minutes=5))}
    ...
    def trigger(self):
        ...
        if os.path.isfile("last_mail"):
            with open("last_mail") as f:
                ...
                if content:
                    dif = now - \
                        datetime.fromtimestamp(int(float(content)))
