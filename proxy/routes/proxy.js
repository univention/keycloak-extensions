 ...
 */
router.use("*/openid-connect/token*", fetchBlockActions, applyBlocks);
    ...
    onProxyRes: responseInterceptor(
      async (responseBuffer, proxyRes, req, res) => {
        if (
          ...
          const rawToken = (
            resCookies["KEYCLOAK_IDENTITY"] ||
            resCookies["KEYCLOAK_IDENTITY_LEGACY"]
          )?.value;
          if (rawToken === undefined) {
            ...
            if (
              res.statusCode === 200 &&
              (req._ipCaptchaActions.rows.length > 0 ||
                req._deviceCaptchaActions.rows.length > 0)
            ) {
